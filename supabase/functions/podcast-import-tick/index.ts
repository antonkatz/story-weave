// Background worker for podcast imports. Scheduled via pg_cron every minute.
...
// verify_jwt is disabled (see config.toml) so pg_cron can call this without credentials.
...
export async function tick(): Promise<{ processed: string | null }> {
  const supabaseAdmin = adminClient();
