    ...
    for (const batch of batches) {
      ...
      if (agent === "structure") {
        ...
        batchActions = await callAgent(
          LOVABLE_API_KEY,
          promptFor("structure"),
          `${bookContext}\n\n# New conversation messages since this agent last ran\n${messagesText}\n\nPropose structure actions. CRITICAL: chapter_id and section_id MUST be one of the ids listed above — never invent ids. When emitting add_section you MUST also fill "purpose" (a 1–2 sentence description of what the section covers) — never propose a section without a purpose. If a section belongs to a chapter that doesn't exist yet, you may emit add_section with chapter_id=null AND fill chapter_title_hint, chapter_synopsis_hint, chapter_theme_hint so the chapter can be auto-created on approval. Reuse the SAME chapter_title_hint string verbatim for multiple sections that belong to the same not-yet-created chapter (so they don't create duplicates). The synopsis is set ONLY at chapter creation time — never propose set_chapter_synopsis. Be conservative: only propose changes the conversation supports and that don't duplicate existing structure.${focusNote}`,
          STRUCTURE_TOOLS,
          "structure_actions",
        );
